...
public static class MeshSlicer
{
    ...
    public class SplitPiece
    {
        ...
        public List<int>[] triGroups = new List<int>[] { new(), new() }; //build surface and interior tri list
        public Dictionary<int, ContourNode> contourNodes = new();
        ...
        public Mesh CompileMesh(Vector3 sliceNormal, Vector3 sliceTangent, Bounds? uvBounds)
        {
            ...
            List<ContourNode> missingNext = new();
            ...
            {
                List<ContourNode> missingPrev = new();
                foreach (ContourNode node in contourNodes.Values)
                {
                    if (node.prev == null)
                    {
    ...
    public static Mesh[] Slice(Mesh mesh, Vector3 slicePosition, Vector3 sliceNormal, Vector3 sliceTangent, Bounds? uvBounds = null, int minVertexOverlap = 0)
    {
        ...
        Plane slice = new(sliceNormal, slicePosition);
        //initialize
    ...
    {
        Vector2 ratioVec2 = new(ratio, ratio);
        if (uvA.x > uvB.x)
        {

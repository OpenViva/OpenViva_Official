...
#include "Assets/3rd Party/Paid/Common/Stylization_URP_PROP.hlsl"
...
half3 RT_MCAP_SUB1(half3 MCapOutP, half4 _MainTex_var, half3 _RTD_MVCOL, out half3 RTD_TEX_COL)
{
		...
		bodyColor = _SkinColor*half3( 1., bodyTex.rr );
		//apply nail colors
		float isHand = step(0.75,bodyTex.g);
		float isFinger = step(0.15,bodyTex.g)*(1.-isHand);
		float neither = step( isHand+isFinger, 0.0 );
		bodyColor = _FingerNailColor*isHand+_ToeNailColor*isFinger+bodyColor*neither;
...
//RT_CA
half3 RT_CA( half3 color, half4 _MainTex_var)
{

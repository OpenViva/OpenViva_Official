...
Shader "Universal Render Pipeline/RealToon/Version 5/Default/Default"
{
    Properties
    {
		...
		_ShadowHardness ("Shadow Hardness", Range(0, 1)) = 1

        _SelfShadowRealtimeShadowIntensity ("Self Shadow & Realtime Shadow Intensity", Range(0, 1)) = 0.9
        _SelfShadowThreshold ("Threshold", Range(0, 1)) = 0.830
...
#pragma target 4.5 //targetol
		...
		#include "Assets/3rd Party/Paid/Common/Stylization_URP_Core.hlsl"
...
#pragma target 4.5 //targetfl
		...
		#pragma shader_feature_local_fragment N_F_NFD_ON
        #pragma shader_feature_local_fragment N_F_SK_ON
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_FL
		...
		) 
        {
			...
			//RT_CA
			float3 RTD_CA = RT_CA(color * SSAmOc + GLO_OUT, _MainTex_var);

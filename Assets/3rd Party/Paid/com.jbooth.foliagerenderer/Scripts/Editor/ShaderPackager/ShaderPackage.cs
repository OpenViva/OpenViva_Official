...
namespace JBooth.FoliageRenderer.ShaderPackager
{
#if __FOLIAGERENDERER_DEVMODE__
    [CreateAssetMenu(fileName = "Shader.foliageRendererpack", menuName = "FoliageRenderer/ShaderPackage", order = 0)]
#endif
    public class ShaderPackage : ScriptableObject
    {
        ...
        [MenuItem("Window/FoliageRenderer/Development Mode/Enable Shader Development")]
        static void EnableDevMode()
        {
            var flags = GetFlags();
            if (!flags.Contains("__FOLIAGERENDERER_DEVMODE__"))
            {
                flags.Add("__FOLIAGERENDERER_DEVMODE__");
                SetFlags(flags);
            }
        }

        [MenuItem("Window/FoliageRenderer/Development Mode/Disable Shader Development")]
        static void DisableDevMode()
        {
            var flags = GetFlags();
            if (flags.Remove("__FOLIAGERENDERER_DEVMODE__"))
                SetFlags(flags);
        }

        // Validate the menu item defined by the function above.
        // The menu item will be disabled if this function returns false.
        [MenuItem("Window/FoliageRenderer/Development Mode/Enable Shader Development", true)]
        static bool ValidateEnableDevMode()
        {
            var flags = GetFlags();
            return !flags.Contains("__FOLIAGERENDERER_DEVMODE__");
        }
        [MenuItem("Window/FoliageRenderer/Development Mode/Disable Shader Development", true)]
        static bool ValidateDisableDevMode()
        {
            return GetFlags().Contains("__FOLIAGERENDERER_DEVMODE__");
        }
#endif
        ...
        {
#if __BETTERSHADERS__ && __FOLIAGERENDERER_DEVMODE__

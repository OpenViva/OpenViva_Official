...
namespace MyProperlyFormedNamespaceName
{
    ...
    public class Snipit_AchievementObject : MonoBehaviour
    {
        ...
        public void Example_UnlockEvent()
        {
            // ... This is not a functional event listener ... just highlighting the event you would use
            myAch.StatusChanged.AddListener(null /*Some Function name*/);

            //This is ran any time the achievement state changes assuming Steam tells the game about it
            //Generally its easier to read teh IsAchieved field on demand but the object is also a GameEvent
            //Meaning its an event stored as an object in memory, its important that you remove listener when done
            //This even persists in memory for as long as the game so if the delegate you assigned it is disposed it 
            //could cause a null reference exception

            myAch.StatusChanged.RemoveListener(null /*Same Function Name you assigned*/);
        }
#endif
    }
}

...
half3 RT_MCAP_SUB1(half3 MCapOutP, half4 _MainTex_var, half3 _RTD_MVCOL, out half3 RTD_TEX_COL)
{
	...
	// Define body color based on _MainTex_var and other parameters
	half3 bodyColor;
	half2 bodyTex = _MainTex_var.rg;

	// Base skin color logic
	bodyColor = _SkinColor * half3(1.0, bodyTex.rr);

	// Apply nail colors based on bodyTex.g
	float isHand = step(0.75, bodyTex.g);           // Check if it's hand region
	float isFinger = step(0.15, bodyTex.g) * (1.0 - isHand); // Check if it's finger region
	float neither = step(isHand + isFinger, 0.0);   // Check if it's neither hand nor finger
	bodyColor = _FingerNailColor * isHand +
				_ToeNailColor * isFinger +
				bodyColor * neither;               // Combine colors based on region

	// Set RTD_TEX_COL with the calculated bodyColor
	RTD_TEX_COL = (bodyColor.rgb * _MaiColPo) * MCapOutP * _RTD_MVCOL;

	// Fallback for _UseSkin: Override bodyColor and RTD_TEX_COL
	if (!_UseSkin)
	{
		bodyColor = _MainColor;
		RTD_TEX_COL = _MainTex_var.rgb * (bodyColor.rgb * _MaiColPo) * MCapOutP * _RTD_MVCOL;
	}

	// Set RTD_MCIALO_IL based on RTD_TEX_COL or fallback
	half3 RTD_MCIALO_IL;
	if (!_MCIALO)
	{
	...
	// Return the final RTD_MCIALO_IL
	return RTD_MCIALO_IL;
...
float2 eyeMove(float2 eyeUv)
{
	eyeUv = (eyeUv-0.5)*_PupilShrink+0.5; 
	eyeUv.x += _PupilRight*_SideMultiplier;
	eyeUv.y -= _PupilUp;
	return eyeUv;
}

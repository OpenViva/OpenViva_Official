Shader "RealToon/Viva"
{
    Properties
    {
	    ...
	    [ToggleOff] _UseSkin ("Use Skin?", Float ) = 0.0
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_OL
		...
		)
        {
			...
			//
			//VIVA SPECIFIC
			bool isCharacter = dot(_SkinColor.rgb, float3(1.0, 1.0, 1.0)) < 3.0;
			//**
			half3 RTD_MMTTO_OO;
			
			half2 bodyTex = _MainTex_var.rg;
			
		    half3 bodyColor = _SkinColor * half3(1.0, bodyTex.rr);

		    float isHand = step(0.75, bodyTex.g);
		    float isFinger = step(0.15, bodyTex.g) * (1.0 - isHand);
			...
			else
			{
				if (isCharacter)
				{
					RTD_MMTTO_OO = _OutlineColor.rgb * bodyColor;
				}
				else
				{
